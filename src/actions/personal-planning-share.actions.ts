 ...
 * avec une secrétaire par exemple") : octroi en lecture seule, sans lien
 ...
 */
export async function shareAgenda(granteeId: string) {
  ...
  const share = await prisma.personalPlanningShare.upsert({
    ...
    update: {},
    create: { ownerId: session.user.id, granteeId },
  });
