...
export function NotificationsSheet({
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      {/* Demande utilisateur — SheetContent est en bg-sidebar (fond marine
          fonce, voir ui/sheet.tsx) mais le texte par defaut (text-foreground)
          reste calibre pour un fond clair : illisible ici sans le forcer en
          text-sidebar-foreground (clair). */}
      <SheetContent side="right" className="flex w-full flex-col text-sidebar-foreground sm:max-w-md">
        ...
        <div className="flex-1 space-y-1 overflow-y-auto px-1">
          {notifications.length === 0 && (
            <p className="p-4 text-sm text-sidebar-foreground/60">Aucune notification.</p>
          )}
          {notifications.map((n) => {
            const tone = NOTIFICATION_TONE_META[toneForNotificationType(n.type)];
            return (
              <NotificationRow key={n.id} notification={n}>
                <div className="rounded-md border border-sidebar-border p-2.5 text-sm">
                  <div className="mb-1 flex items-center gap-1.5">
                    ...
                    <span className="ml-auto whitespace-nowrap text-[10px] text-sidebar-foreground/60">
                      {new Date(n.createdAt).toLocaleString("fr-FR")}
                    </span>
                  </div>
                  {n.lien ? (
                    <Link href={n.lien} onClick={() => setOpen(false)} className="font-medium text-sidebar-foreground hover:underline">
                      {n.titre}
                    </Link>
                  ) : (
                    <span className="font-medium text-sidebar-foreground">{n.titre}</span>
                  )}
                </div>
              </NotificationRow>
            );
        ...
        <Link
          ...
          className="block rounded-md border border-sidebar-border p-2 text-center text-sm text-sidebar-foreground hover:bg-sidebar-accent"
        >
