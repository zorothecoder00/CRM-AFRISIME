...
import { addProcessusDocument } from "@/actions/processus.actions";
import {
...
export function ProcessusDocumentFormDialog({ processusId }: { processusId: string }) {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<AddProcessusDocumentInput>({
    resolver: zodResolver(addProcessusDocumentSchema),
    defaultValues: { processusId },
  });
  const { run: submit, isPending } = useAction(addProcessusDocument, {
    successMessage: "Document ajouté.",
  });

  async function onSubmit(data: AddProcessusDocumentInput) {
    const result = await submit({ ...data, processusId });
    if (result.ok) {
      reset({ processusId });
